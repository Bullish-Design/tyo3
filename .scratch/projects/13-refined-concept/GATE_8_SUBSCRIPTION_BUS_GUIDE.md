...
> - the reverse-dependency index already in the code graph
>   (`graph.transitive_dependents` for the id-level closure; `graph._file_importers`,
>   the `file → {importer files}` map maintained by the native `CodeDelta` applier)
>   for the §4.3.3 closure,
...
existing `graph.transitive_dependents` (id-level) and `graph._file_importers`
(file-level) reverse-dep surface — not a native change.
...
- **Closure (§4.3.3).** `affected = changed ∪ deleted ∪ transitive_dependents(changed ∪
  deleted)` using the head graph's reverse-dep index — `graph.transitive_dependents(id)`
  for the id-level closure (call it per id in `changed ∪ deleted` and union), or
  `graph._file_importers` (the `file → {importer files}` map) mapped through file→ids.
  **No global scan** (§4.3.3): the closure walks only inbound edges from the changed set.
...
- **If the id-level closure is hard to get from the graph**, compute it from
  `graph._file_importers` (file-level) mapped through file→ids rather than adding head
  access in the bus; keep the bus a pure consumer of the already-built reverse-dep index.
