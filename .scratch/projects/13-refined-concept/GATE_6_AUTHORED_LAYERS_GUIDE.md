...
- **SyncResult gains an `authored` field.** Add `authored: Vec<String>` to
  `dto::SyncResultDto` and the Pydantic `SyncResult` (`src/tyo3/models/analysis.py`),
  ...
  - **Leave `code_delta` unset (post-Gate-3N).** `SyncResultDto` now carries a
    `code_delta: Option<CodeDeltaDto>` (the native structural delta the head graph is
    rebuilt from). An authored write must leave it `None` — `..Default::default()` does
    this. The Python applier (`CodeGraph.apply_delta` / `session._apply_graph_delta`)
    already short-circuits when `code_delta is None`, so an authored edit touches the
    head graph not at all and does not force the lazy graph to build.
...
**Build & validate.**
- Test: at a pinned R, for some id, `snap.graph().symbol(id).content_hash`,
