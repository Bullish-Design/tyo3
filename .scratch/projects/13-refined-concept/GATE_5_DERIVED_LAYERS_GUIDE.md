...
rust/src/
  entity.rs / dto          # per-profile content hashes on the CodeDelta node DTO
                           #   (SymbolNodeDto), NOT the document_symbols read DTO (Step 1)
src/tyo3/tests/
...
**Files.** `rust/src/entity.rs` + the **CodeDelta node DTO** (`SymbolNodeDto` in Rust;
its Python mirror `SymbolNodeDelta` in `src/tyo3/models/analysis.py`);
`src/tyo3/derive/cache.py` (Python); the graph node payload
(`src/tyo3/graph/models.py`) and its applier (`CodeGraph._node_from_delta`).

> **Data-path note (post-Gate-3N).** The code graph is now built **exclusively from the
> native `CodeDelta`** (`code_delta_full()` → `apply_code_delta` → `_node_from_delta`);
> it no longer reads the per-file `document_symbols` read-surface DTO. So the carrier
> for per-profile hashes is the **`CodeDelta` node payload** (`SymbolNodeDto` /
> `SymbolNodeDelta`), exactly where the existing single `content_hash` already rides —
> **not** the `document_symbols` symbol DTO. Putting the map on the read DTO would never
> reach `SymbolNode`.
...
- **Rust — per-profile hashes (gated).** In `extract_entities`, compute the entity
  ...
  on the **`SymbolNodeDto`** (the CodeDelta node entry) a small map
  `content_hashes: {profile_name: hash_hex}` alongside the existing default
  `content_hash` (which stays the identity/reconciliation hash, unchanged). **With no
  derived layers, the only profile is the default**, so this is exactly today's single
  hash at zero extra cost — the no-config no-op.
- The graph node (`SymbolNode`) carries `content_hashes` through from the **delta node**:
  add the field to `SymbolNodeDelta` and copy it in `CodeGraph._node_from_delta`
  (the same place `content_hash=nd.content_hash` is set). Still a small payload — a
  handful of 128-bit hashes, no vectors/text, §6.2.2.
...
def resolve_input(self, layer, snapshot, durable_id) -> tuple[GenInput, str]:
    if layer.is_code_derived:
        node = snapshot.graph().symbol(durable_id)   # CodeGraph.symbol(id) -> SymbolNode
...
- Test (cross-layer consistency, §10.2.2 derived half): at a pinned R,
  `snap.graph().symbol(id).content_hash`, `snap.derived("embeddings", id)`, and
